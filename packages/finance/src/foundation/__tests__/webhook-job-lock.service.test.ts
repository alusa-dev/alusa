...
vi.mock('@alusa/database', () => ({
  prisma: {
    $queryRaw: webhookJobLockMock.queryRaw,
...
describe('webhookJobLockService', () => {
  ...
  it('adquire lock criando registro novo', async () => {
    webhookJobLockMock.queryRaw.mockResolvedValue([{ jobName: 'webhook-scheduler:global' }]);
    ...
    expect(webhookJobLockMock.queryRaw).toHaveBeenCalledTimes(1);
  });
  ...
  it('retorna busy quando lock ainda está válido', async () => {
    ...
    webhookJobLockMock.queryRaw.mockResolvedValue([]);
