          ...
          <SupportPanel title="Webhooks relacionados">
            <div className="space-y-3">
              {charge.webhooks.map((webhook) => (
                <RowLink
                  key={webhook.id}
                  href={`/contas/${resolvedParams.contaId}/webhooks/${webhook.id}`}
          ...
          <SupportPanel title="Ações seguras">
            <div className="space-y-3">
              <SupportSafeActionButton
                ...
                endpoint="/api/admin/actions/reconcile-charge"
                payload={{ contaId: resolvedParams.contaId, chargeId: resolvedParams.chargeId }}
              />
              <SupportSafeActionButton
                label="Consultar status Asaas"
                endpoint="/api/admin/actions/check-asaas-status"
                payload={{ contaId: resolvedParams.contaId, chargeId: resolvedParams.chargeId }}
              />
              <SupportSafeActionButton
                label="Obter links oficiais"
                endpoint="/api/admin/actions/refresh-charge-links"
                payload={{ contaId: resolvedParams.contaId, chargeId: resolvedParams.chargeId }}
              />
              <SupportSafeActionButton
                label="Marcar divergência"
                endpoint="/api/admin/actions/divergence"
