...
export default async function SupportEnrollmentPage({ params }: { params: Promise<{ contaId: string; matriculaId: string }> }) {
  ...
  const session = await requireAdminSessionForPage(`/contas/${resolvedParams.contaId}/matriculas/${resolvedParams.matriculaId}`);
          ...
          <SupportPanel title="Cobranças da matrícula">
            <div className="space-y-3">
              {matricula.cobrancas.map((charge) => (
                <RowLink
                  key={charge.id}
                  href={`/contas/${resolvedParams.contaId}/financeiro/cobrancas/${charge.asaasPaymentId ?? charge.id}`}
                  title={charge.id}
