...
export default async function SupportResponsavelPage({ params }: { params: Promise<{ contaId: string; responsavelId: string }> }) {
  ...
  const session = await requireAdminSessionForPage(`/contas/${resolvedParams.contaId}/responsaveis/${resolvedParams.responsavelId}`);
          ...
          <SupportPanel title="Alunos vinculados">
            <div className="space-y-3">
              {responsavel.alunos.map((item) => (
                <RowLink
                  key={item.aluno.id}
                  href={`/contas/${resolvedParams.contaId}/alunos/${item.aluno.id}`}
