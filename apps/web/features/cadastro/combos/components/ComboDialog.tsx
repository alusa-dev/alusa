...
export function ComboDialog({ open, mode, contaId, combo, onOpenChange, onSubmit }: Props) {
  ...
  const baseInputCls =
    'h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 placeholder:text-slate-400 focus-visible:border-[#5c2f91] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#5c2f91]/40 md:h-10';
  const textAreaCls =
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !submitting && onOpenChange(o)}>
      <DialogContent
        fullScreenMobile
        className="w-full max-w-3xl gap-0 overflow-hidden bg-slate-50 p-0 max-md:flex max-md:h-[100dvh] max-md:max-h-[100dvh] max-md:flex-col max-md:min-h-0"
      >
          ...
          <section className="space-y-5 rounded-2xl border border-slate-200 bg-slate-50 px-5 py-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="flex flex-col gap-1">
                ...
                >
                  <SelectTrigger className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 md:h-10">
