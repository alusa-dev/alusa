...
import { NOTIFICATION_SECTIONS, type ChannelConfig } from '@/features/configuracoes/notificacoes/asaas/constants';
      ...
      <div className="container mx-auto max-w-7xl px-4 py-6 pb-8">
        ...
        <div className="space-y-8">
          ...
          <CustomerNotificationsEditor
            ...
            initialPreferences={aluno.notificacoes.preferences}
          />
...
function CustomerNotificationsEditor({
  alunoId,
  customerId,
  initialPreferences,
}: {
  ...
  initialPreferences: CustomerNotificationPreference[];
}) {
  const initialNormalized = useMemo(
    () => initialPreferences.map(normalizeNotificationPreference),
    [initialPreferences],
  );
  const [preferences, setPreferences] =
    useState<CustomerNotificationPreference[]>(initialNormalized);
  ...
  const loadCustomerNotifications = useCallback(async () => {
    if (!customerId) {
      setPreferences(initialNormalized);
      setOriginalPreferences(initialNormalized);
      setLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/alunos/${alunoId}/notificacoes?customerId=${encodeURIComponent(customerId)}`,
        { cache: 'no-store' },
      );
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
  ...
  }, [alunoId, customerId, initialNormalized]);
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/alunos/${alunoId}/notificacoes`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <section className={sectionClass}>
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-600">
            Configuração individual do customer no Asaas para cobranças atuais e próximas.
          </p>
          {customerId ? (
      ...
      {!customerId ? (
        <div className="rounded-lg border border-slate-200 bg-white px-4 py-8 text-center text-sm text-slate-500">
          Nenhum customer Asaas vinculado para configurar notificações.
        </div>
      ) : null}
