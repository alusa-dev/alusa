...
import { useCallback, useEffect, useRef, useState, type CSSProperties, type ElementType, type PointerEvent as ReactPointerEvent } from 'react';
...
function PublicPdf({ url, fields, escolaNome, signature, signedFieldId, bottomInset = 0, interactive = true, onFieldClick: handleFieldClick, onPageCount }: { url: string; fields: Field[]; escolaNome: string; signature: Signature | null; signedFieldId: string | null; bottomInset?: number; interactive?: boolean; onFieldClick: (_field: Field) => void; onPageCount?: (_pages: number) => void }) {
  ...
  return (
    <div className="relative h-full min-h-0 min-w-0 w-full overflow-hidden bg-slate-100">
      <div ref={viewportRef} className={cn('h-full min-h-0 w-full overscroll-contain', isMobile ? 'touch-pan-y overflow-y-auto overflow-x-hidden [-webkit-overflow-scrolling:touch]' : 'overflow-auto touch-auto')}>
        {!components || pageWidth === 0 ? (
          ...
          <div className="flex min-h-full items-start justify-center bg-slate-100 px-3 py-6 sm:px-6 sm:py-8 lg:px-16 lg:py-10" style={{ width: stageWidth, paddingBottom: isMobile ? Math.max(bottomInset + 24, 240) : undefined }}>
                  ...
                  <div key={page} className="relative shrink-0 overflow-hidden bg-white shadow-[0_12px_28px_-18px_rgba(15,23,42,0.38)] [&>div]:block" style={{ width: renderedPageWidth }} aria-label={`Página ${page}`}>
                    ...
                    {fields.filter((field) => field.pagina === page).map((field) => {
                      ...
                      return <div key={field.id} className="absolute overflow-visible" style={{ left: `${field.x * 100}%`, top: `${field.y * 100}%`, width: `${field.largura * 100}%`, height: `${field.altura * 100}%` }}>
                        {isSchool ? <span className="pointer-events-none absolute inset-0 z-10 flex max-w-full items-center justify-center overflow-hidden text-ellipsis whitespace-nowrap px-1 text-center font-serif text-[8px] italic leading-none text-slate-900 lg:px-2 lg:text-base">{escolaNome || 'Escola'}</span> : interactive ? <button type="button" onClick={() => handleFieldClick(field)} className={cn('absolute inset-0 z-10 flex min-h-11 items-center justify-center rounded-md border px-2 text-center text-[11px] font-semibold transition-colors', isSigned ? 'border-transparent text-transparent' : 'cursor-pointer border-amber-300 bg-amber-50/90 text-amber-900 shadow-sm hover:bg-amber-100')}>{isSigned ? 'Editar assinatura' : (field.tipo === 'RUBRICA' ? 'Rubrica' : 'Clique para assinar')}</button> : <span className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center rounded px-2 text-center text-[11px] font-semibold text-slate-500">Campo de assinatura</span>}
...
function ConsentDocumentPage({ term, isLast, className, style }: { term: ConsentTerm; isLast: boolean; className: string; style?: CSSProperties }) {
  return <article className={className} style={style}>
    <div className="space-y-6 sm:space-y-8">
      <h1 className="text-left text-lg font-bold uppercase tracking-[0.04em] text-slate-950 sm:text-xl">{term.titulo}</h1>
      <div className="space-y-5 sm:space-y-6">
        {term.texto.split(/\n\s*\n/).map((paragraph, paragraphIndex) => {
          ...
          return isHeading
            ? <h2 key={`${term.id}-heading-${paragraphIndex}`} className="text-left text-sm font-bold uppercase tracking-[0.02em] text-slate-950">{content}</h2>
            : <p key={`${term.id}-paragraph-${paragraphIndex}`} className="whitespace-pre-line text-left">{content}</p>;
        })}
      </div>
    </div>
    {!isLast && <div className="mt-12 border-t border-slate-200 pt-4 text-xs text-slate-400">Próximo termo de consentimento</div>}
  </article>;
}

function ConsentDocumentPreview({ terms, bottomInset = 0 }: { terms: ConsentTerm[]; bottomInset?: number }) {
  const [viewportWidth, setViewportWidth] = useState(0);
  const viewportRef = useRef<HTMLDivElement | null>(null);
  ...
  const pageWidth = Math.max(280, Math.min(760, viewportWidth - 24));
  const stageWidth = Math.max(viewportWidth, pageWidth + 24);

  return <>
    <div className="relative h-full min-h-0 w-full overflow-hidden bg-slate-100 lg:hidden">
      <div ref={viewportRef} className="h-full min-h-0 w-full touch-pan-y overflow-y-auto overflow-x-hidden overscroll-y-contain [-webkit-overflow-scrolling:touch]">
        <div className="flex min-h-full items-start justify-center bg-slate-100 px-3 py-6" style={{ width: stageWidth, paddingBottom: Math.max(bottomInset + 24, 240) }}>
          <div className="flex flex-col items-center gap-6">
            {terms.map((term, index) => <ConsentDocumentPage key={term.id} term={term} isLast={index === terms.length - 1} className="min-h-[680px] shrink-0 bg-white p-5 text-[13px] leading-6 text-slate-800 shadow-[0_12px_28px_-18px_rgba(15,23,42,0.38)]" style={{ width: pageWidth }} />)}
          </div>
        </div>
      </div>
    </div>
    <div className="hidden lg:mx-auto lg:block lg:w-full lg:max-w-[760px] lg:space-y-5">
      {terms.map((term, index) => <ConsentDocumentPage key={term.id} term={term} isLast={index === terms.length - 1} className="min-h-[680px] w-full bg-white p-[2.54cm] text-[15px] leading-7 text-slate-800 shadow-[0_10px_28px_-18px_rgba(15,23,42,0.7)]" />)}
    </div>
  </>;
}
...
function ConsentChoices({ terms, answers, compact = false, onChange }: { terms: ConsentTerm[]; answers: Record<string, 'AUTORIZADO' | 'RECUSADO'>; compact?: boolean; onChange: (_termId: string, _decision: 'AUTORIZADO' | 'RECUSADO') => void }) {
  return <div className={cn(compact ? 'flex min-w-0 items-center gap-2 overflow-x-auto' : 'space-y-3')}>
    {terms.map((term) => <div key={term.id} className={cn(compact ? 'flex min-w-[300px] shrink-0 items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2' : 'rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm')}>
...
export function ContratoPublicoFeature({ token, kind = 'academic' }: { token: string; kind?: 'academic' | 'event' }) {
  ...
  const formatDataNascimento = useCallback((value: string) => { const digits = value.replace(/\D/g, '').slice(0, 8); if (digits.length <= 2) return digits; if (digits.length <= 4) return `${digits.slice(0, 2)}/${digits.slice(2)}`; return `${digits.slice(0, 2)}/${digits.slice(2, 4)}/${digits.slice(4)}`; }, []);
  ...
  useEffect(() => {
    ...
    if (!dock) return;
    const updateHeight = () => setMobileDockHeight(dock.getBoundingClientRect().height);
    updateHeight();
  ...
  return (
    <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-slate-100 text-slate-900">
      ...
      <main className="flex min-h-0 flex-1 flex-col bg-slate-100 px-0 pb-0 lg:overflow-hidden lg:bg-transparent">
        <div className="relative z-50 flex min-h-14 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-6 lg:-mb-14 lg:ml-[228px] lg:px-8">
        ...
        <div className="flex w-full min-h-0 flex-1 flex-col lg:grid lg:grid-cols-[228px_minmax(0,1fr)] lg:overflow-hidden">
          ...
          <div className={cn('relative flex min-h-0 min-w-0 flex-col overflow-hidden bg-slate-100 lg:border-r lg:border-slate-200 lg:pt-14', step === 4 && 'hidden')}>
            <section aria-label={step === 2 ? 'Termo de consentimento' : 'Documento para assinatura'} className={cn('min-h-0 min-w-0 w-full max-w-full flex-1 shrink overscroll-contain', step === 2 ? 'overflow-hidden px-0 py-4 pb-24 sm:px-6 sm:py-6 sm:pb-24 lg:overflow-y-auto lg:px-8 lg:py-8 lg:pb-24' : 'overflow-hidden', step === 4 && 'hidden')}>
              {step === 2 ? <ConsentDocumentPreview terms={consentimentos} bottomInset={mobileDockHeight} /> : <PublicPdf url={contrato.arquivoPdfUrl} fields={contrato.camposAssinatura} escolaNome={contrato.escolaNome} signature={signature} signedFieldId={signedFieldId} bottomInset={mobileDockHeight} interactive={step === 3} onFieldClick={openSignature} onPageCount={setDocumentPages} />}
            </section>
          </div>

          <aside ref={mobileDockRef} className={cn(step === 4 ? 'fixed inset-x-0 top-28 bottom-0 z-40 mx-auto max-h-none overflow-hidden bg-slate-100 px-3' : 'fixed inset-x-0 bottom-0 z-40 mx-auto max-h-[70vh] overflow-y-auto bg-slate-100', 'max-w-[760px] rounded-none lg:min-h-0 lg:max-h-none lg:overflow-y-auto lg:rounded-none', step === 4 && 'max-w-none px-0', step === 1 || step === 2 || step === 3 ? 'lg:hidden' : 'lg:sticky lg:top-0 lg:flex lg:h-full lg:w-full lg:max-w-[480px] lg:items-center lg:justify-self-center lg:bg-transparent lg:pb-20 lg:pt-14')}>
            <Card className={cn('overflow-hidden rounded-none border-slate-200 bg-white shadow-none lg:min-h-0 lg:rounded-none lg:border-0 lg:bg-transparent', step === 4 && 'mx-auto flex h-full w-full max-w-none flex-col lg:max-w-[480px]')}>
