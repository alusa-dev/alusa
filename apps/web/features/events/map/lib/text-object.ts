...
export function measureTextWidth(
  ...
  if (!ctx) return Math.max(fontSize, text.length * fontSize * 0.55);

  const family = fontFamily.split(',')[0]?.trim().replace(/['"]/g, '') ?? 'Inter';
  ...
  for (const line of lines) {
    ...
    maxWidth = Math.max(maxWidth, metrics.width + letterSpacing * Math.max(0, line.length - 1));
  }
  return maxWidth;
}
