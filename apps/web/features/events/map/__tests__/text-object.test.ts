...
describe('text-object', () => {
  ...
  it('supports combined underline and strikethrough decorations', () => {
    expect(getKonvaTextDecoration({ underline: true, lineThrough: true })).toBe('underline line-through');
    expect(getKonvaTextDecoration({ textDecoration: 'underline' })).toBe('underline');
  });
