  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        ...
        >
          <div>
            <p className="text-sm font-semibold text-gray-900">Aniversários do mês</p>
            <p className="mt-1 text-xs capitalize text-gray-500">{getMonthLabel(todayMonth)}</p>
          </div>

          <div className="mt-4 flex-1 space-y-2.5">
            {visibleStudents.length === 0 ? (
              <p className="text-sm leading-6 text-gray-500">Nenhum aniversariante ativo neste mês.</p>
            ) : (
              visibleStudents.map((student) => (
                <div key={student.id} className="flex items-center gap-2.5">
                  ...
                  <span className="shrink-0 rounded-full bg-[#f4ecfd] px-2 py-0.5 text-[11px] font-medium text-[#383242]">
          ...
          <div className="mt-4 flex items-center justify-between text-xs text-gray-500">
            ...
            <span className="font-medium text-[#383242]">Ver calendário</span>
          </div>
        </button>
      </DialogTrigger>
      ...
      <DialogContent className="max-w-[850px] gap-0 overflow-hidden p-0 data-[state=open]:animate-modal-expand-in data-[state=closed]:animate-modal-shrink-out sm:max-w-[850px]">
        <DialogHeader className="border-b border-gray-100 px-6 py-5 text-left">
          <div>
            <DialogTitle className="text-base font-semibold text-slate-900">Aniversariantes</DialogTitle>
          ...
          <div className="flex flex-col items-center border-b border-gray-100 p-6 sm:w-[340px] sm:shrink-0 sm:border-b-0 sm:border-r">
            <div className="w-fit">
              <Calendar
                ...
                modifiersClassNames={{
                  birthday:
                    'bg-[#f4ecfd] text-[#4c1d95] font-semibold rounded-md [&_button]:bg-[#f4ecfd] [&_button]:text-[#4c1d95] [&_button]:font-semibold [&_button]:rounded-md [&_button]:scale-90',
                }}
            ...
            <div className="mt-5 flex w-full flex-wrap items-center justify-center gap-x-5 gap-y-2 text-xs text-slate-500">
              <span className="flex items-center gap-2">
                <span className="h-3 w-3 rounded-full bg-[#f4ecfd] ring-1 ring-[#d8b4fe]" />
              ...
              {viewedBirthdays.length === 0 ? (
                ...
                viewedBirthdays.map((student) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium leading-tight text-slate-900">{student.nome}</p>
                      <p className="mt-0.5 text-xs text-slate-500">
                        Aniversário em {String(student.dia).padStart(2, '0')}/{String(student.mes).padStart(2, '0')}
                      </p>
                    </div>

                    <span className="shrink-0 rounded-full bg-[#f4ecfd] px-2.5 py-1 text-xs font-medium text-[#4c1d95]">
