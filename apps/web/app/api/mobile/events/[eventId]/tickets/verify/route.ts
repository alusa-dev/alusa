...
import { verifyMobileTicket } from '@/features/events/server/mobile-events.service';
import {
  authenticate,
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const limiter = rateLimit(`mobile-events:ticket-verify:${actor.userId}:${ipFromRequest(request)}`, 120, 10 * 60 * 1000);
  if (!limiter.ok) {
    return response({ error: { code: 'RATE_LIMITED', message: 'Muitas tentativas. Aguarde alguns minutos.' } }, 429);
  }

  const parsed = bodySchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success || !eventId?.trim()) {
    return response({ error: { code: 'INVALID_INPUT', message: 'Informe um código de ingresso válido.' } }, 422);
  }

  try {
    return response(await verifyMobileTicket(actor, eventId, parsed.data.ticketCode, parsed.data.confirm));
  } catch (error) {
