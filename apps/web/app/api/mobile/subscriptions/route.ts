...
export async function GET(request: Request) {
  ...
  const limiter = rateLimit(`mobile-subscriptions:list:${actor.userId}:${ipFromRequest(request)}`, 60, 10 * 60 * 1000);
  if (!limiter.ok) {
    return NextResponse.json(
  ...
  try {
    const data = await listMobileSubscriptions({
    ...
    return NextResponse.json(
      { error: { code: 'SERVER_ERROR', message: 'Não foi possível carregar as assinaturas.' } },
