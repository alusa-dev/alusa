...
import { alunoRouteParamsDTOSchema } from '@/features/cadastro/alunos/dtos';
...
export async function getAlunoDetalhesRoute(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const auth = await resolveTenantSession();
    if (!auth.ok) {
      return jsonError(401, 'NAO_AUTENTICADO', 'Usuário não autenticado');
    }
    const { id } = alunoRouteParamsDTOSchema.parse(await params);

    const aluno = await prisma.aluno.findFirst({
      where: { id, contaId: auth.contaId },
      include: {
    ...
    const [alunoCustomer, alunoStandaloneCharges, alunoStandalonePlans, alunoStandaloneSubscriptions] =
      await Promise.all([
        ...
        findCustomerForPayer(auth.contaId, 'ALUNO', aluno.id),
        prisma.charge.findMany({
          where: {
            ...
            OR: [
              ...
              { sale: { contaId: auth.contaId, alunoId: aluno.id } },
            ],
          },
        ...
        prisma.standaloneInstallmentPlan.findMany({
          where: {
            ...
            OR: [
              { payerType: 'ALUNO', payerId: aluno.id },
              { sales: { some: { contaId: auth.contaId, alunoId: aluno.id } } },
            ],
          },
          orderBy: { createdAt: 'desc' },
        }),
        prisma.standaloneSubscription.findMany({
          where: { contaId: auth.contaId, payerType: 'ALUNO', payerId: aluno.id },
          orderBy: { createdAt: 'desc' },
        }),
      ]);
      ...
      matriculaIds.length
        ? prisma.matriculaFamiliarItem.findMany({
            where: {
              ...
              matriculaFamiliar: { contaId: auth.contaId },
            },
