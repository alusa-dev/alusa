...
import { responsavelRouteParamsDTOSchema } from '@/features/responsaveis/dtos';
...
export async function getResponsavelOverviewRoute(
  _request: Request,
  context: { params: Promise<{ id: string }> | { id: string } },
) {
  const user = await resolveTenantSession();
  if (!user.ok) {
  ...
  try {
    const { id } = responsavelRouteParamsDTOSchema.parse(await Promise.resolve(context.params));
