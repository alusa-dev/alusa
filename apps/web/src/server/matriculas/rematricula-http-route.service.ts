...
import { getAcademicDateDifference } from '@alusa/shared/date-only';
...
async function resolveAuthContext(explicit?: string | null) {
  const tenant = await resolveTenantSession(explicit);
  if (!tenant.ok) {
    return {
      contaId: null,
      mismatch: tenant.reason === 'CONTA_MISMATCH',
      sessionContaId: null,
      user: null,
    };
  }
  ...
  return {
    ...
    mismatch: false,
    sessionContaId: tenant.contaId,
    user: { id: tenant.userId, role: tenant.role, contaId: tenant.contaId },
  };
}
...
export async function getRematriculasRoute(req: Request) {
  try {
...
export async function postRematriculasRoute(req: Request) {
  try {
    ...
    return jsonError(500, 'ERRO_CRIAR_REMATRICULA', 'Não foi possível criar a rematrícula.');
  }
}
