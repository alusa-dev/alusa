/**
 * Testes unitários para /api/finance/installments
 *
...
import { GET, POST } from '@/app/api/finance/installments/route';
...
vi.mock('@alusa/finance', async () => {
  ...
  return {
    ...actual,
    getKycSummary: vi.fn(),
    createStandaloneInstallmentPlan: vi.fn(),
    listInstallmentPlans: vi.fn(),
  };
});

const { getServerSession } = await import('next-auth');
const { createStandaloneInstallmentPlan, listInstallmentPlans, getKycSummary } = await import('@alusa/finance');
...
function makePostReq(body: unknown) {
  const url = new URL('http://localhost:3001/api/finance/installments');
...
describe('API Finance Installments', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('GET: 401 quando não autenticado', async () => {
    ...
    const res = await GET(makeGetReq('http://test/api/finance/installments'));
  ...
  it('GET: 400 quando query inválida', async () => {
    ...
    const res = await GET(makeGetReq('http://test/api/finance/installments?status=INVALID'));
    expect(res.status).toBe(400);

    const json = await res.json();
    expect(json).toMatchObject({ error: { code: 'VALIDATION_ERROR' } });

    expect(listInstallmentPlans).not.toHaveBeenCalled();
  });

  it('GET: aplica filtro por status e paginação', async () => {
    mockSession({ id: 'u1', contaId: 'c1', role: 'FINANCEIRO' });

    vi.mocked(listInstallmentPlans).mockResolvedValueOnce({ total: 12, items: [] } as never);

    const res = await GET(makeGetReq('http://test/api/finance/installments?page=2&pageSize=5&status=ACTIVE'));

    expect(res.status).toBe(200);
    expect(listInstallmentPlans).toHaveBeenCalledWith({
      contaId: 'c1',
      limit: 5,
      offset: 5,
      status: 'ACTIVE',
  ...
  it('POST: 400 quando body inválido', async () => {
    ...
    expect(createStandaloneInstallmentPlan).not.toHaveBeenCalled();
  });

  it('POST: 403 quando feature flag desabilitada', async () => {
    mockSession({ id: 'u1', contaId: 'c1', role: 'FINANCEIRO' });

    vi.mocked(createStandaloneInstallmentPlan).mockResolvedValueOnce({
      success: false,
